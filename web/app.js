...
function renderSituationFields(situation, primaryFilled) {
    ...
    root.appendChild(
      el(
        ...
        'Сроки, которые считаются независимо от цепочки обжалования и от категории ' +
          ...
          'жалобы — месяц со дня его вынесения (ч. 1 ст. 379.2 ГПК), обжалование ' +
          'определения об утверждении мирового соглашения, заключаемого в процессе ' +
          'исполнения судебного акта, — месяц со дня его вынесения, сразу в кассацию, ' +
          'минуя апелляцию (ч. 11 ст. 153.10 ГПК). Кассационная жалоба на судебный ' +
          'приказ — три месяца со дня вступления обжалуемого акта в законную силу ' +
          '(ч. 1 ст. 376.1 ГПК), тоже минуя апелляцию; эта дата не вводится, а ' +
          'вычисляется от даты получения копии приказа должником. Сроки третейского ' +
          'разбирательства — в ситуации «Третейский суд», признание и исполнение ' +
          'решений иностранных судов — в своей одноимённой ситуации. Заполните нужную ' +
          'дату.',
      ),
    );
  }
  if (situation.id === 'child_cases') {
    root.appendChild(
      el(
        'p',
        null,
        'Две категории дел, у которых сроки обжалования короче общего порядка. Дела о ' +
          'возвращении ребёнка и об осуществлении прав доступа на основании ' +
          'международного договора РФ (глава 22.2 ГПК): по десять рабочих дней и на ' +
          'апелляцию (ч. 1 ст. 244.17), и на частную жалобу (ч. 1 ст. 244.18) — даты ' +
          'независимы, заполните нужную. Дела об усыновлении (удочерении) ребёнка ' +
          '(глава 29 ГПК): апелляция на решение об удовлетворении заявления — десять ' +
          'рабочих дней со дня принятия решения в окончательной форме (ч. 2.1 ст. 274), ' +
          'а не месяц общего порядка; при отказе в удовлетворении заявления действует ' +
          'общий месячный срок (ст. 321). Выберите категорию — появятся её даты.',
      ),
    );
  }
  if (situation.id === 'arbitration') {
    root.appendChild(
      el(
        'p',
        null,
        'Сроки, связанные с третейским разбирательством. Отмена постановления ' +
          'третейского суда о компетенции — месяц со дня его ПОЛУЧЕНИЯ стороной, а не ' +
          'вынесения (ч. 2 ст. 422.1 ГПК). Заявление об отмене решения третейского суда ' +
          '(глава 46 ГПК) — три месяца, но точка отсчёта зависит от субъекта: со дня ' +
          'получения решения стороной третейского разбирательства (ч. 2 ст. 418) либо со ' +
          'дня, когда об оспариваемом решении узнало или должно было узнать лицо, не ' +
          'являющееся стороной, в отношении прав и обязанностей которого оно вынесено, ' +
          'либо прокурор (ч. 3 ст. 418) — выберите вариант переключателем. Кассационная ' +
          'жалоба на определения по делам об оспаривании решений третейских судов и о ' +
          'выдаче исполнительного листа на их принудительное исполнение — три месяца со ' +
          'дня вступления обжалуемого определения в законную силу (ч. 1 ст. 376.1 ГПК), ' +
          'сразу в кассацию, минуя апелляцию. Сроки независимы друг от друга: по одному ' +
          'делу их может понадобиться несколько, заполните нужные.',
      ),
    );
  }
  if (situation.id === 'foreign_judgment') {
    root.appendChild(
      el(
        'p',
        null,
        'Признание и исполнение решений иностранных судов (глава 45 ГПК) — два ' +
          'отдельных срока, по одному на каждый вид решения. Решение, требующее ' +
          'принудительного исполнения (ст. 409–412), предъявляется к исполнению в ' +
          'течение трёх лет со дня вступления его в законную силу (ч. 3 ст. 409 ГПК). ' +
          'Против признания решения, которое принудительного исполнения не требует ' +
          '(ст. 413–415), возражения подаются в течение месяца со дня, когда ' +
          'заинтересованное лицо о решении УЗНАЛО, — не со дня вынесения решения и не ' +
          'со дня вступления его в силу (ч. 2 ст. 413 ГПК); этот срок ст. 416 ГПК ' +
          'распространяет и на решения иностранных третейских судов (арбитражей). ' +
          'Сроки независимы друг от друга, заполните нужную дату.',
      ),
    );
  }
  ...
  if (situation.id === 'enforcement') {
    root.appendChild(
      el(
        'p',
        null,
        'Исполнительный документ уже на руках, и нужен только срок предъявления его к ' +
