...
async function downloadICS() {
  ...
  if (isIOS()) {
    ...
    // (иначе это уже вложение, а не переход по адресу).
    //
    // Изначально здесь была навигация на blob:-URL — в обычном Safari этого
    // достаточно: WebKit сам распознаёт content-type блоба и предлагает
    // импорт в Календарь. Но в Яндекс.Браузере на том же iPhone это не
    // сработало: файл для импорта не открывался. Похоже, у него собственный
    // менеджер загрузок, который перехватывает переход на blob:-ссылку до
    // того, как до неё доберётся штатное распознавание content-type
    // (согласуется с более ранним наблюдением: тот же браузер предлагал
    // «Сохранить в Файлы» вместо системного листа при попытке через
    // navigator.share). blob: — это специфичная для вкладки ссылка на
    // объект в памяти, а не обычный URL с заголовками ответа, и, видимо,
    // именно поэтому сторонний менеджер загрузок считает её «своей» —
    // перехватывает раньше, чем это делает WebKit.
    //
    // data:-URI с явным MIME-типом ведёт себя иначе: это не ссылка на
    // объект вкладки, а сами данные, закодированные прямо в URL, — их не за
    // что «перехватывать» менеджеру загрузок, и content-type виден сразу
    // как часть адреса. На практике это доводит до кнопки «Добавить в
    // календарь» и в Safari, и в Яндекс.Браузере.
    const url = `data:${ICS_TYPE_DOWNLOAD},${encodeURIComponent(ics)}`;
    window.location.href = url;
    // revokeObjectURL здесь не нужен: data:-URI не ссылается на Blob и не
    // держит объект в памяти вкладки — освобождать нечего (в отличие от
    // blob:-варианта ниже, для остальных платформ).
    return;
  }
