...
| web/app.js | 300 | `const d = new Date();` |
| web/app.js | 301 | `return `${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}`;` |
| web/app.js | 1351 | `const ics = buildICS(currentIcsTerms, { referenceDate: today, now: new Date() });` |
