...
| web/app.js | 294 | `const d = new Date();` |
| web/app.js | 295 | `return `${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}`;` |
| web/app.js | 1314 | `const ics = buildICS(currentIcsTerms, { referenceDate: today, now: new Date() });` |
