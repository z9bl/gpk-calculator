...
| web/app.js | 290 | `const d = new Date();` |
| web/app.js | 291 | `return `${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}`;` |
| web/app.js | 1323 | `const ics = buildICS(currentIcsTerms, { referenceDate: today, now: new Date() });` |
