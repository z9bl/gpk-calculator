...
| web/app.js | 248 | `const d = new Date();` |
| web/app.js | 249 | `return `${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}`;` |
| web/app.js | 1088 | `const ics = buildICS(currentIcsTerms, { referenceDate: today, now: new Date() });` |
