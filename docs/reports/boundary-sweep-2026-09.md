...
| web/app.js | 296 | `const d = new Date();` |
| web/app.js | 297 | `return `${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}`;` |
| web/app.js | 1316 | `const ics = buildICS(currentIcsTerms, { referenceDate: today, now: new Date() });` |
