...
| web/app.js | 285 | `const d = new Date();` |
| web/app.js | 286 | `return `${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}`;` |
| web/app.js | 1324 | `const ics = buildICS(currentIcsTerms, { referenceDate: today, now: new Date() });` |
