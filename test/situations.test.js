...
test('приказное производство: возражения и кассация — одна цепочка на одном якоре', () => {
  ...
  // Второй, почтовый вход — тоже у ОБОИХ узлов. Фикция п. 32 ПП ВС РФ № 62
  // определяет начало течения десятидневного срока на возражения (ст. 128), а
  // не кассационного срока напрямую, поэтому по одной лишь дате прибытия
  // отправления появляется и карточка возражений. Прежде фикция была подключена
  // только к кассации, и здесь ожидался один узел — это была ошибка применения
  // нормы, а не свойство ситуации.
...
test('судебный приказ (кассация): узел в приказном производстве, а не в пуле отдельных сроков', () => {
  ...
  // Вариант (b) — только дата прибытия на почту — вход обоих узлов: фикция
  // п. 32 ПП ВС РФ № 62 применяется на уровне возражений (ст. 128), кассация
  // считается от их результата.
  const viaPostal = buildView(
    { sudebny_prikaz_postal_arrival_date: '2025-07-04' },
    { today: '2025-07-01' },
  );
