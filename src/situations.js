...
export const SITUATIONS = [
  ...
  },
  {
    ...
    // Короткий вход для случая, когда исполнительный документ уже на руках:
    // цепочка обжалования не нужна, нужен только срок предъявления к
    // исполнению (ст. 21 ФЗ № 229-ФЗ) с перерывами (ч. 1-3 ст. 22) и вычетами
    // (ч. 3.1 ст. 22). Существующие узлы предъявления внутри ветвей
    // (enforcement_presentation и его копии, court_order_presentation,
    // periodic_payments_presentation) при этом остаются на месте: это ещё один
    // вход к тому же расчёту, а не их замена.
    //
    ...
    //
    // В `fields` перечислено только то, что принадлежит именно этой ситуации:
    // сам dropdown и новое поле даты для варианта «вступившее в силу решение».
    // Два других варианта якорятся на УЖЕ существующие поля своих ситуаций —
    // court_order_issued_date (судебный приказ) и
    // periodic_payment_period_end_date (периодические платежи). Они
    // переиспользуются, а не дублируются: одна и та же дата не должна
    // вводиться дважды и расходиться между двумя входами к одному расчёту.
    // Поэтому здесь их нет — иначе поле оказалось бы закреплено за двумя
    // ситуациями сразу (инвариант «поля ввода не дублируются между
    // ситуациями», situations.test.js). На экране их рисует
    ...
    fields: ['enforcement_document_type', 'enforcement_decision_entry_into_force_date'],
