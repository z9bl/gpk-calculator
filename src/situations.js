...
export const SITUATIONS = [
  ...
  },
  {
    ...
    //
    // Два узла одной цепочки должника: возражения (ст. 128) от дня получения
    // копии приказа и, если возражения не поданы и приказ вступил в силу,
    // кассационная жалоба на приказ (п. 1 ч. 2 ст. 377, ч. 1 ст. 376.1) — уже
    // от РЕЗУЛЬТАТА первого узла: окончание срока на возражения и есть момент
    // вступления приказа в силу (единая точка правды, см. computeCourtOrderTerms
    // в chain.js).
    //
    // Кассация переехала сюда из пула «Отдельные сроки»: там она стояла в
    // отрыве от возражений и со своим полем даты получения копии
    // (sudebny_prikaz_received_date), дублировавшим court_order_copy_received_date
    // — одна и та же дата вводилась дважды в двух разных ситуациях. Дубль
    // убран, поле осталось одно. Перенос — перегруппировка, а не правка
    // расчёта: норма, длительность и порядок вывода даты вступления приказа в
    // силу у узла те же (см. SUDEBNY_PRIKAZ_CASSATION в chain.js).
    //
    // sudebny_prikaz_postal_arrival_date — второй, почтовый вход ОБОИХ узлов:
    // дата получения копии выводится из даты прибытия отправления и
    // семидневного срока хранения (п. 32 ПП ВС РФ от 27.12.2016 № 62). Поля
    // взаимоисключающие, приоритет у прямой даты (resolveSudebnyPrikazReceivedDate
    // в chain.js). Фикция п. 32 относится к началу течения десятидневного срока
    // на возражения (ст. 128), поэтому по одной лишь почтовой дате появляются
    // обе карточки — возражения и кассация от их результата. Прежде фикция была
    // подключена только к кассации, и карточка возражений в этом случае не
    // появлялась.
    //
    // Третий узел этой ситуации, предъявление приказа к исполнению
  ...
  },
  {
    ...
    // где оно что-то открывает. В «Приказном производстве» осталась своя
    // дата-якорь (court_order_copy_received_date) и два своих узла — возражения
    // должника и кассационная жалоба на приказ.
    //
  ...
  },
  {
    ...
    // жалоба — рабочие дни, все остальные узлы этого пула — месяц или три
    // месяца (ч. 1, 2 ст. 108).
    // Из этого пула выделены три ситуации: «Третейский суд» (четыре узла),
    // «Признание и исполнение решений иностранных судов» (два узла главы 45) —
    // см. ниже — и кассационная жалоба на судебный приказ, ушедшая в «Приказное
    // производство» выше, к возражениям должника: оба узла считаются от одного
    // якоря (дня получения копии приказа) и образуют одну цепочку, а порознь
    // дублировали ввод этой даты. Во всех трёх случаях перенос был
    // перегруппировкой, а не правкой расчёта: нормы, якоря и поля узлов не
    // изменились.
    // *_restoration_circumstance_date — годичный потолок восстановления
    // (ч. 7 ст. 112 ГПК РФ), см. общий комментарий у ветви 'general' выше;
    // здесь — у единственного узла прямой кассации пула, для которого он
    // предусмотрен (settlement_approval_cassation_appeal).
    fields: [
  ...
  },
  {
    ...
    // взаимоисключающими полями, как у sudebny_prikaz_* в приказном
    // производстве, — приоритет за received_date (вариант (a), сторона третейского
