...
export const SITUATIONS = [
  ...
  },
  {
    ...
    //
    // Два узла одной цепочки должника, оба от одной даты-якоря — дня получения
    // копии приказа: возражения (ст. 128) и, если возражения не поданы и приказ
    // вступил в силу, кассационная жалоба на приказ (п. 1 ч. 2 ст. 377,
    // ч. 1 ст. 376.1).
    //
    ...
    //
    // sudebny_prikaz_postal_arrival_date — второй, почтовый вход ТОЛЬКО
    // кассации: дата получения копии выводится из даты прибытия отправления и
    ...
    // в chain.js). Возражения этой фикции не применяют — их норма при переносе
    // не менялась, поэтому по одной лишь почтовой дате узел возражений не
    // появляется.
    //
