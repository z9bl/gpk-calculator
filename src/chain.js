...
// открылся кассационный срок) и читает ту же дату-якорь
// court_order_copy_received_date. Узел переехал сюда из пула «Отдельные сроки»
// вместе со своим вторым, почтовым входом; его расчёт при переносе не
// менялся.
//
...
//
// Поле варианта (a) — то же court_order_copy_received_date, что и у
// COURT_ORDER_OBJECTION (ст. 128): это один и тот же юридический факт —
// день получения должником копии приказа, — и своего дубля
// sudebny_prikaz_received_date у кассации больше нет. Прежде поле было своим,
// потому что узел жил в другой ситуации («Отдельные сроки»), и одну и ту же
// дату приходилось вводить дважды; теперь оба узла в «Приказном производстве»
// и читают одно поле. Расчёт при этом не изменился: приоритет прямой даты над
// почтовой и вся арифметика ниже — те же.
//
// Вариант (b) остаётся собственным входом кассации: COURT_ORDER_OBJECTION
// фикцию п. 32 ПП ВС РФ № 62 не применяет и считает только от введённой даты
// получения — его норма здесь намеренно не трогается.
//
