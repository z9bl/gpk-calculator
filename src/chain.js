 ...
 * SPEC.md), решение о них принимает владелец проекта:
 *
 *   1. Несколько отрезков складываются все без исключения (totalDeductedDays в
 *      core/engine/deduction.js). Норма говорит об одном отрезке.
 *   2. Отрезок «со дня A до дня B» = B − A: день начала входит, день окончания
 *      нет (periodDays там же).
 *   3. Вычет и перерыв (ч. 1–3) применяются независимо: перерыв сдвигает якорь
...
export const DEDUCTION_ASSUMPTION = {
  ...
  norm: 'ч. 3.1 ст. 22 ФЗ № 229-ФЗ',
  text:
    'Расчёт по ч. 3.1 опирается на допущения, которые из текста нормы прямо не ' +
    'следуют и требуют юридической проверки: несколько периодов складываются все ' +
    'без исключения; длина периода считается как разность дат (день предъявления ' +
    'входит, день окончания — нет); вычет и перерыв срока (ч. 1–3) применяются ' +
    'независимо — перерыв даёт полный трёхлетний срок, из которого затем ' +
    'вычитается сумма всех периодов.',
};
...
// трёхлетний срок, из которого здесь вычитается сумма отрезков (ASSUMPTION 3,
// см. DEDUCTION_ASSUMPTION выше).
function withDeductions(result, term, deductions) {
  return genericWithDeductions(result, term, enforcementDeductions(deductions), DEDUCTION_CONFIG);
}
